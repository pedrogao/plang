...
class Intepretor extends AstVisitor {
  ...
  visitFunctionCall(functionCall: FunctionCall): any {
    ...
    if (functionCall.sym != null) {
      ...
      console.log(
        "Runtime error, cannot find declaration of " + functionCall.name + "."
      );
      return;
    }
  }
  ...
  if (fileName.endsWith(".ts")) {
    ...
    // 用vm运行程序，或者输出字节码文件
      ...
      if (args.mode == RunningMode.vm) {
        ...
        let date2 = new Date();
        console.log(
          "耗时：" + (date2.getTime() - date1.getTime()) / 1000 + "秒"
        );
      }
...
function parseCommandArgs(): CommandArgs | null {
  ...
  if (args.fileName != null) {
    args.fileNameWithoutPostfix = args.fileName.substr(
      0,
      args.fileName.length - 3
    );
...
function showHelp() {
  ...
  console.log(
    "\tnode play --dumpBC example.ts -> 编译成字节码执行，并保存到.bc文件中"
  );
  console.log(
