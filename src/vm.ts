...
import { SimpleType, UnionType, FunctionType, Type, SysTypes } from "./types";
...
export class BCGenerator extends AstVisitor {
  ...
  visitIfStatement(ifstmt: IfStatement): any {
    ...
    let offset_elseBlock: number =
      code_condition.length + code_ifBlock.length + 6; //else语句块的地址
      ...
      switch (opCode) {
        ...
        case OpCode.invokestatic:
          ...
          if (functionSym.name == "println") {
            ...
            //传递参数
            let paramCount = (functionSym.theType as FunctionType).paramTypes
              .length;
