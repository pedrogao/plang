    ...
    if (symInScope != null) {
      if (declaredSyms.has(variable.name)) {
        ...
        if (symInScope.kind == SymKind.Variable) {
          this.addError(
            "Variable: '" + variable.name + "' is used before declaration.",
            variable
          );
        } else {
          this.addError(
  ...
  visitBinary(binary: Binary): any {
    if (Operators.isAssignOp(binary.op) || binary.op == Op.Dot) {
      ...
      if (!binary.exp1.isLeftValue) {
        this.addError(
          "Left child of operator " + Op[binary.op] + " need a left value",
          binary.exp1
        );
      }
  ...
  visitUnary(u: Unary): any {
    ...
    if (u.op == Op.Inc || u.op == Op.Dec) {
      ...
      if (!u.exp.isLeftValue) {
        this.addError(
          "Unary operator " + Op[u.op] + "can only be applied to a left value",
          u
        );
      }
  ...
  visitBinary(bi: Binary): any {
    ...
    if (Operators.isAssignOp(bi.op)) {
      ...
      switch (bi.op) {
        ...
        default:
          this.addError(
            "Unsupported binary operator: " + Op[bi.op] + "in ConstFolder",
            bi
          );
      }
      bi.op = v;
  ...
  visitUnary(u: Unary): any {
    ...
    if (typeof v1 != "undefined") {
      if (u.op == Op.Inc) {
        ...
        this.addError(
          "Unsupported unary operator: " + Op[u.op] + "in ConstFolder",
          u
        );
      }
    }
