...
abstract class Inst {
  ...
  patchComments(str: string): string {
    if (this.comment != null) {
      ...
      else if (str.length < 19) str += "\t\t";
      else if (str.length < 21) str += "\t";
...
class Inst_2 extends Inst {
  ...
  toString(): string {
    let str =
      OpCode[this.op] +
      "\t" +
      this.oprand1.toString() +
      ", " +
      this.oprand2.toString();
    return this.patchComments(str);
  }
...
export class AsmGenerator extends AstVisitor {
  ...
  private isTempVar(oprand: Oprand) {
    if (this.s.functionSym != null) {
      return (
        oprand.kind == OprandKind.varIndex &&
  ...
  visitProg(prog: Prog): any {
    ...
    this.asmModule?.numTotalVars.set(
      this.s.functionSym,
      this.s.nextTempVarIndex
    );
  ...
  visitFunctionDecl(functionDecl: FunctionDecl): any {
    ...
    this.asmModule?.isLeafFunction.set(
      this.s.functionSym as FunctionSymbol,
      true
    );
  ...
  visitReturnStatement(rtnStmt: ReturnStatement): any {
    if (rtnStmt.exp != null) {
      ...
      if (this.tailAnalysisResult != null) {
        if (
          this.tailAnalysisResult.tailRecursives.indexOf(
            rtnStmt.exp as FunctionCall
          ) != -1
        ) {
          this.getCurrentBB().insts.push(
            new Inst_1(
              OpCode.jmp,
              new Oprand(OprandKind.bb, this.s.bbs[0]),
              "Tail Recursive Optimazation"
            )
          );
        } else if (
          this.tailAnalysisResult.tailCalls.indexOf(
            rtnStmt.exp as FunctionCall
          ) != -1
        ) {
          ...
          this.getCurrentBB().insts.push(
            new Inst_1(
              OpCode.tailCallJmp,
              new Oprand(OprandKind.label, "_" + functionName),
  ...
  visitIfStatement(ifStmt: IfStatement): any {
    ...
    if (bbElseBlock != null) {
      //跳转到else块
      instConditionJump = new Inst_1(
        op,
        new Oprand(OprandKind.bb, bbElseBlock)
      );
    } else {
      //跳转到if之后的块
      instConditionJump = new Inst_1(
        op,
        new Oprand(OprandKind.bb, bbFollowing)
      );
    }
    ...
    if (bbElseBlock != null) {
      //如果没有else块，就不需要添加跳转了。
      let instIfBlockJump = new Inst_1(
        OpCode.jmp,
        new Oprand(OprandKind.bb, bbFollowing)
      );
  ...
  private getJumpOpCode(compOprand: Oprand): OpCode {
    ...
    if (compOprand.value == Op.G) {
      ...
      console.log(
        "Unsupported compare operand in conditional expression: " +
          compOprand.value
      );
    }
    return op;
  ...
  visitForStatement(forStmt: ForStatement): any {
    ...
    if (compOprand != null) {
      //如果没有循环条件，就会直接落到循环体中
      let op = this.getJumpOpCode(compOprand);
      let instConditionJump = new Inst_1(
        op,
        new Oprand(OprandKind.bb, bbFollowing)
      );
      bbCondition.insts.push(instConditionJump);
    ...
    if (compOprand != null) {
      ...
      //如果没有循环条件，就直接回到循环体的第一句
      bbDest = bbBody;
    }
    let instBodyJump = new Inst_1(
      OpCode.jmp,
      new Oprand(OprandKind.bb, bbDest)
    );
  ...
  visitVariableDecl(variableDecl: VariableDecl): any {
    if (this.s.functionSym != null) {
      ...
      let varIndex = this.s.functionSym.vars.indexOf(
        variableDecl.sym as VarSymbol
      );
  ...
  visitBinary(bi: Binary): any {
    ...
    switch (bi.op) {
      ...
      default:
        console.log(
          "Unsupported OpCode in AsmGenerator.visitBinary: " + Op[bi.op]
        );
    }
  ...
  visitUnary(u: Unary): any {
    ...
    if (u.op == Op.Inc || u.op == Op.Dec) {
      ...
      insts.push(
        new Inst_2(opCode, new Oprand(OprandKind.immediate, 1), tempVar)
      );
    ...
    //-
      ...
      //用0减去当前值
      insts.push(
        new Inst_2(OpCode.movl, new Oprand(OprandKind.immediate, 0), tempVar)
      );
  ...
  visitVariable(variable: Variable): any {
    if (this.s.functionSym != null && variable.sym != null) {
      return new Oprand(
        OprandKind.varIndex,
        this.s.functionSym.vars.indexOf(variable.sym)
      );
    }
  }
  ...
  private callIntrinsics(intrinsic: string, args: Oprand[]): any {
    ...
    //把结果放到一个新的临时变量里
    if (functionType.returnType != SysTypes.Void) {
      //函数有返回值时
  ...
  visitFunctionCall(functionCall: FunctionCall): any {
    ...
    if (this.tailAnalysisResult != null) {
      if (this.tailAnalysisResult.tailRecursives.indexOf(functionCall) != -1) {
        ...
        this.tailAnalysisResult.tailCalls.indexOf(functionCall) != -1
      ) {
        isTailCall = true;
      }
    }
    ...
    insts.push(
      new Inst_1(
        op,
        new FunctionOprand(functionCall.name, args, functionType.returnType)
      )
    );
...
class Lower {
  ...
  private lowerFunction(
    ...
    let isLeafFunction = this.asmModule.isLeafFunction.get(
      functionSym
    ) as boolean;
    if (isLeafFunction) {
      let bytes =
        this.spillOffset +
        this.numArgsOnStack * 8 +
        this.usedCalleeProtectedRegs.length * 8;
    ...
    let tailCall =
      lastBB.insts.length > 0 &&
      lastBB.insts[lastBB.insts.length - 1].op == OpCode.tailCallJmp;
    if (!tailCall) {
      ...
      if (bb.insts.length > 0) {
        ...
        if (lastInst.op == OpCode.tailCallJmp) {
          ...
          bb.insts.push(
            new Inst_1(
              OpCode.jmp,
              new Oprand(OprandKind.bb, bbEndPoint),
              lastInst.comment
            )
          );
    ...
    if (!this.canUseRedZone) {
      ...
      if (this.rspOffset > 0)
        newInsts.push(
          new Inst_2(
            OpCode.subq,
            new Oprand(OprandKind.immediate, this.rspOffset),
            Register.rsp
          )
        );
    }
  ...
  private lowerBBLabelAndJumps(
    ...
    for (let i = 0; i < newBBs.length; i++) {
      ...
      if (
        OpCodeHelper.isJump(lastInst.op) &&
        (lastInst as Inst_1).oprand.kind == OprandKind.bb
      ) {
        //jump指令
        let jumpInst = lastInst as Inst_1;
  ...
  private lowerBB(bb: BasicBlock, newInsts: Inst[]) {
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      //1个操作数
        ...
        if (inst.op != OpCode.declVar) {
          //忽略变量声明的伪指令。
          //处理函数调用
          //函数调用前后，要设置参数；
          if (
            inst_1.op == OpCode.callq ||
            inst_1.op == OpCode.tailRecursive ||
            inst_1.op == OpCode.tailCall
          ) {
            let liveVarsAfterCall =
              i == insts.length - 1
                ? (this.livenessResult.initialVars.get(bb) as number[])
                : (this.livenessResult.liveVars.get(insts[i + 1]) as number[]);
            varsToSpill = this.lowerFunctionCall(
              inst_1,
              liveVars,
              liveVarsAfterCall,
              newInsts
      ...
      //没有操作数
        ...
        if (inst.op == OpCode.reload) {
          ...
          if (
            i != insts.length - 1 &&
            !OpCodeHelper.isReturn(insts[i + 1].op)
          ) {
  ...
  private lowerFunctionCall(
    ...
    if (saveCallerProtectedRegs) {
      for (let varIndex of liveVarsAfterCall) {
        ...
        if (
          oprand.kind == OprandKind.register &&
    ...
    if (args.length > 6) {
      ...
      for (let j = 6; j < numArgs; j++) {
        ...
        let oprand = this.lowerOprand(
          liveVars,
          args[j],
          newInsts,
          args[j].kind == OprandKind.varIndex
        );
        newInsts.push(
          new Inst_2(OpCode.movl, oprand, new MemAddress(Register.rsp, offset))
        );
      }
    }
  ...
  private lowerOprand(
    ...
    if (aa) console.log(newOprand);

    if (aa && !found) console.log(this.loweredVars);
  ...
  private spillVar(
    ...
    newInsts.push(
      new Inst_2(OpCode.movl, reg, address, "spill\tvar" + varIndex)
    );
  ...
  private reloadVar(varIndex: number, newInsts: Inst[]): Register | null {
    ...
    if (oprand.kind == OprandKind.memory) {
      ...
      this.assignRegToVar(varIndex, reg);
      newInsts.push(
        new Inst_2(OpCode.movl, address, reg, "reload\tvar" + varIndex)
      );
      return reg;
    }
    return null;
  ...
  private spillARegister(newInsts: Inst[]): Register | null {
    for (let varIndex of this.loweredVars.keys()) {
      ...
      if (
        oprand.kind == OprandKind.register &&
        this.reservedRegisters.indexOf(oprand as Register) != -1
      ) {
  ...
  private assignRegToVar(varIndex: number, reg: Register) {
    ...
    if (
      Register.calleeProtected32.indexOf(reg) != -1 &&
      this.usedCalleeProtectedRegs.indexOf(reg) == -1
    ) {
  ...
  private getFreeRegister(liveVars: number[]): Register | null {
    ...
    for (let reg of Register.registers32) {
      if (
        allocatedRegisters.indexOf(reg) == -1 &&
        this.reservedRegisters.indexOf(reg) == -1
      ) {
        result = reg;
        break;
      }
  ...
  private saveCalleeProtectedRegs(newInsts: Inst[]) {
    for (let i = 0; i < this.usedCalleeProtectedRegs.length; i++) {
      let regIndex = Register.calleeProtected32.indexOf(
        this.usedCalleeProtectedRegs[i]
      );
...
class CFG {
  ...
  private buildCFG() {
    //构建edgesOut;
    for (let i: number = 0; i < this.bbs.length - 1; i++) {
      //最后一个基本块不用分析
      let bb = this.bbs[i];
  ...
  private analyzeFunction(bbs: BasicBlock[], result: LivenessResult) {
    ...
    while (bbsToDo.length > 0) {
      ...
      let liveVars =
        bb.insts.length == 0
          ? []
          : (result.liveVars.get(bb.insts[0]) as number[]);
      ...
      if (typeof fromBBs != "undefined") {
        for (let bb2 of fromBBs) {
          ...
          if (!this.isSubsetOf(liveVars, liveVars2)) {
            if (bbsToDo.indexOf(bb2) == -1) bbsToDo.push(bb2);
